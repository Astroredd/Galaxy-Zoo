...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(deit_model.parameters(), lr=lr)
    ...
    trained_model, all_labels, all_preds, train_losses, val_losses = train_model(
        deit_model, train_loader, val_loader, criterion, optimizer, num_epochs=num_epochs, scheduler=scheduler, patience=patience
    )
    ...
    # Guardar el modelo
    torch.save(trained_model.state_dict(), 'best_deit_model.pth')
    
    # Guardar las etiquetas y predicciones para la matriz de confusión
    np.save('all_labels_deit.npy', all_labels)
    np.save('all_preds_deit.npy', all_preds)

    # Guardar las pérdidas de entrenamiento y validación
    np.save('train_losses_deit.npy', train_losses)
    np.save('val_losses_deit.npy', val_losses)

    print('Modelo y métricas guardados correctamente.') 

    # Cargar las pérdidas guardadas
    train_losses = np.load('train_losses_deit.npy')
    val_losses = np.load('val_losses_deit.npy')
